            ...
            for t in sugar.keys():
                ...
                for speed in sugar[t]:
                    ...
                    if speed["is_regex"]:
                        ...
                        if speed["regex"].search(p[t]) != None:
...
def eat(score, processes, detected, flags):
    ...
    #i'm sorry for hard-coding this
    fields = [
        {
            "name": "BeatmapID: {}".format(beatmap_id),
            "value": "[Download Beatmap](https://bm.cookiezi.tk/web/osu-search.php?{})".format(beatmap_id),
            "inline": True
        },
        {
            "name": "ScoreID: {}".format(score.scoreID),
            "value": "[Download Replay](http://osuhow.tk/web/replays/{})".format(score.scoreID),
            "inline": True
        }
    ]
...
#We don't want to store their process list unless matching
    if do_restrict:
        ...
        glob.db.execute("INSERT INTO cakes(id, userid, score_id, processes, detected, flags) VALUES (NULL,%s,%s,%s,%s,%s)", [score.playerUserID, score.scoreID, json.dumps(tag_list), json.dumps(tag_list), flags])
